...
ANC captures protocol fees generated from Anchor. Anchor's protocol fees are sourced from the flow into the yield reserve which comes from bAsset rewards, excess yield, and collateral liquidation fees. `ANC Purchase Factor` (currently 10%) of this value flowing into the yield reserve is used to purchase ANC tokens. These ANC are then redistributed to ANC stakers. `ANC Purchase Factor` can be adjusted through governance. 
